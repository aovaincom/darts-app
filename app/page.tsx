...
// --- STATS MODAL (Integroitu tähän tiedostoon varmuuden vuoksi) ---
                ...
                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                    {tab === 'x01' ? (
                        <>
                            ...
                            <div className="h-64 w-full bg-slate-900 p-2 rounded border border-slate-700">
                                <h3 className="text-sm text-gray-400 mb-2">Progress</h3>
                                {x01Data.length > 1 ? (
                                    <LineChart width={600} height={200} data={x01Data}>
                                        <CartesianGrid strokeDasharray="3 3" opacity={0.2} />
                                        <XAxis dataKey="gameIndex" />
                                        <YAxis domain={['auto', 'auto']} />
                                        <Tooltip contentStyle={{backgroundColor: '#1e293b'}} />
                                        <Line type="monotone" dataKey="rolling" stroke="#4ade80" strokeWidth={2} dot={false} />
                                    </LineChart>
                                ) : <div className="text-center text-gray-500 mt-10">Play more games</div>}
                            </div>
                        </>
                    ) : (
                        <div className="text-center text-gray-400">RTC Stats display (Simple)</div>
                    )}
                </div>
            </div>
        </div>
...
export default function Home() {
  ...
  // --- GAME STATE (Kaikki tila tässä tiedostossa) ---
  ...
  const handleX01Throw = (score: number, multiplier: number) => {
      ...
      setPlayers(prev => {
          ...
          if (p.isBot) return prev; // Estä ihmisen klikkaus botin vuorolla
          ...
          if (bust) {
              ...
              return newPlayers; // Näytä väärä heitto hetken
          }
  ...
  const handleRTCThrow = (hit: boolean) => {
      ...
      setPlayers(prev => {
          ...
          if (p.rtcFinished) {
              // Jos pelaaja on jo valmis, skipataan
               setIsProcessing(true);
               setTimeout(() => {
                   p.currentVisit = []; // Reset visual
  ...
  const handleWin = (winner: PlayerState, currentPlayers: PlayerState[]) => {
      ...
      if (settings.matchMode === 'legs' && winner.legsWon >= settings.targetToWin) matchWon = true;
      // Yksinkertaistettu sets-logiikka tässä fixissä
      ...
      if (matchWon) {
          ...
          setTimeout(() => {
              setPlayers(prev => {
                  ...
                  const nextStarter = (legStarterIndex + 1) % resetP.length;
      ...
      if (currentPlayer?.isBot && !isProcessing) {
          const timer = setTimeout(() => {
              ...
              if (settings.gameMode === 'x01') {
                   ...
                   // Simuloidaan botin heitto UI:ssa 'köntässä'
                   setPlayers(prev => {
                       ...
                       if (bot.scoreLeft === 0 && !bust) {
                           ...
                           setIsProcessing(true); // Estä tuplaloop
                           setTimeout(() => {
                               setPlayers(nextPrev => {
                  ...
                  // RTC Bot
                   setPlayers(prev => {
                       ...
                       bot.currentVisit = Array(3).fill({score:0, multiplier:1, totalValue:0}); // Visual dummy
  ...
  }, [currentPlayerIndex, gameStarted, isProcessing, matchResult]); // Players dependency removed to avoid loops, index controls it
        ...
        if (!p.isBot && p.profileId) {
            if (settings.gameMode === 'x01') {
                updates.push({ id: p.profileId, stats: { 
                    ...
                    // Lisää muut statsit tarvittaessa
                }});
            } else {
                updates.push({ id: p.profileId, stats: {
                    rtcGamesPlayed: 1,
                    ...
                    <div className="space-y-2 max-h-60 overflow-y-auto mb-4">
                        {profiles.map(p => (
                            <div key={p.id} className={`p-3 rounded flex justify-between items-center cursor-pointer border ${selectedProfileIds.includes(p.id) ? 'bg-green-900/50 border-green-500' : 'bg-slate-700 border-transparent'}`} onClick={() => {
                                ...
                                <div><div className="font-bold">{p.name}</div><div className="text-xs text-gray-400">Avg: {getAverage(p)}</div></div>
                            ...
                            <div className="flex justify-between">
                                <div>
                                    ...
                                    <div className="text-xs text-gray-400 font-mono">
                                        {settings.gameMode==='x01' ? `Avg: ${p.stats.average}` : `Darts: ${p.stats.rtcDartsThrown}`}
                                    </div>
                                    {/* Visit dots */}
                                ...
                                <div className="text-right">
                                    <div className="text-3xl font-mono font-bold">{settings.gameMode==='x01' ? p.scoreLeft : (p.rtcFinished?'DONE':p.rtcTarget)}</div>
                                    <div className="text-xs text-gray-500">L: {p.legsWon}</div>
                                </div>
                            </div>
                        </div>
                    ))}
